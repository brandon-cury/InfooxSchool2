<template>
  <a :href="'/accessBook/' + book.slug" class="card rounded-0" style="width: 40px;">
      <DefaultImageBook v-if="imageAffiche" :src="images[0].path" :alt="book.title" :class="'card-img-top rounded-0 mb-2'" />
        <DefaultImageBook v-else :alt="book.title" :class="'card-img-top rounded-0 mb-2'" />
    <div class="card-body p-0">


      <div v-if="book.price" class="card-text oneLine">
        <span class="fw-bolder me-1 bg-danger text-white px-1">{{ calculatePrice(book.price, '1 semaine') }} Fcfa</span>
        <span class="text-danger"> Le plus bas: <i class="text-decoration-line-through">{{ book.price }} Fcfa</i></span>
      </div>
      <div v-else class="card-text">
        <span class="fw-bolder me-3 text-danger">Gratuit</span>
      </div>

      <h4 class="card-title mt-2 twoLine">{{ book.title }}</h4>
      <div class="d-flex justify-content-between">

        <div v-if="book.star > 0" style="color: #D5B42E">
...
const fetchImages = async () => {
    if (props.book.images && props.book.images.length > 0) {
      images.value = props.book.images.sort((a, b) => a.sort - b.sort);
